...
def plot_trajectory(robot_traj, line_values, goal, traj_length=-1, collision=-1):
    ...
    lc = LineCollection(
        segments, cmap=plt.get_cmap("cool"), norm=plt.Normalize(0, line_values.max())
    )
    lc.set_array(line_values)
    lc.set_linewidth(3)
    plt.gca().add_collection(lc)
    img = plt.imread(
        "/home/nathan/HERDR/tsne/City_top_down.png"
    )  # /Users/NathanDurocher/Documents/GitHub/HERDR/
    size = 185
    ...
    xmin, xmax = max(
        [min([robot_traj_array[:, 1].min() - 50, goal[1] - 50]), -200]
    ), min([max([robot_traj_array[:, 1].max() + 50, goal[1] + 50]), 200])
    ymin, ymax = max(
        [min([robot_traj_array[:, 0].min() - 50, goal[0] - 50]), -200]
    ), min([max([robot_traj_array[:, 0].max() + 50, goal[0] + 50]), 200])
    plt.xlim([xmin, xmax])
    plt.ylim([ymin, ymax])
...
def plot_actions(position, line_values, location, GOAL, frame=None):
    ...
    points = np.expand_dims(
        np.array([position[:, :, 1].numpy(), position[:, :, 0].numpy()]).T, 2
    ).transpose(1, 0, 2, 3)
...
def count_data_ratio(loader):
    ...
    print(
        f"Total Samples: {total}, # Positive: {positive} Ratio of total:positive {ratio:.4f}"
    )
    return ratio
...
def moveimages(h5file_path, recursive=False):
    ...
    if load_all_files:
        ...
        data = np.concatenate(
            [loadfromfile(str(h5dataset_fp.resolve())) for h5dataset_fp in files]
        )
    else:
