...
class HERDR(nn.Module):
    def __init__(self, Horizon=1, RnnDim=64):
        ...
        #     nn.Linear(256, 128),
        # )
        self.action_pre = nn.Sequential(nn.Linear(2, 16), nn.ReLU(), nn.Linear(16, 16))
        self.init_hidden = nn.Sequential(
            nn.Linear(128, 128), nn.ReLU(), nn.Linear(128, 2 * self.rnndim)
        )
        self.model_out = nn.Sequential(
            nn.Linear(self.rnndim, 32), nn.ReLU(), nn.Linear(32, 1)
        )
        self.lstm = nn.LSTM(
            input_size=16, hidden_size=self.rnndim, num_layers=1, batch_first=False
        )
...
class HERDR_Resnet(HERDR):
    def __init__(self, Horizon=1, RnnDim=64):
        ...
        self.obs_pre.fc = nn.Sequential(
            nn.LazyLinear(512), nn.ReLU(), nn.Linear(512, 128), nn.ReLU()
        )
