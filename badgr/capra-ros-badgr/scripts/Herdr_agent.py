...
class Herdragent(object):
    ...
    def __init__(self, training=False, recording=None, model_name=None, test_block=1):
        ...
        self.control_pub = rospy.Publisher(
            "/carla/ego/vehicle_control_cmd", CarlaEgoVehicleControl, queue_size=10
        )
        ...
        if self.training:
            ...
            self.spawn_location_file = (
                f"/home/nathan/HERDR/spawn_locations_test{test_block}.txt"
            )
        self.load_spawns()
    ...
    def reset(self, tf=None, goal=None, orca_test=False):
        ...
        if orca_test:
            transform = tf
            self.GOAL = torch.tensor([goal.location.x, goal.location.y]).repeat(
                self.batches, 1, 1
            )
            y = tf.location.y
    ...
    def calc_score_model(self):
        self.rot = (
            torch.tensor(self.vehicle.get_transform().rotation.yaw).unsqueeze(0)
            / 180
            * np.pi
        )
        ...
        goalReward = (goalReward - goalReward.min()) / (
            goalReward.max() - goalReward.min()
        )
        self.score = (
            self.goal_gain * goalReward + self.event + self.action_gain * action_cost
        )
        """ ******* FOR DEBUGGING  ******** """
    ...
    def calculate_position(self):
        ...
        for i in range(0, self.horizon - 1):
            batch_state[:, 0, i + 1] = (
                batch_state[:, 0, i]
                + (1 / self.control_freq)
                * torch.cos(batch_state[:, 3, i])
                * self.actions[:, i, 0]
            )
            batch_state[:, 1, i + 1] = (
                batch_state[:, 1, i]
                + (1 / self.control_freq)
                * torch.sin(batch_state[:, 3, i])
                * self.actions[:, i, 0]
            )
            batch_state[:, 3, i + 1] = (
                batch_state[:, 3, i]
    ...
    def step(self):
        ...
        if self.recording != None:
            cv2.imwrite(
                f"./old_carla_images/{self.recording}/{now}.jpg",
                self.frame.permute(1, 2, 0).numpy(),
            )
